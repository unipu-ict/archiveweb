...
function selectBoxDynamic($field_name='',$tableName='setting',$colom='value',$selected='',$attr='')
{
  ...
  if($query->num_rows() > 0) {
    ...
    $res.='<option value="">Odaberi</option>';
...
function ajx_dataTable($table='',$columns=array(),$Join_condition='', $where = '')
{
  ...
  foreach ($output_arr['data'] as $key => $value)
  {
    ...
    if(CheckPermission($table, "own_view")){
      $output_arr['data'][$key][count($output_arr['data'][$key]) - 1] .= '<a sty id="btnViewRow" class="modalButton2 mClass"  href="javascript:;" type="button" data-src="'.$id.'" title="View"><i class="fa fa-search" data-id=""></i></a>';
    }
