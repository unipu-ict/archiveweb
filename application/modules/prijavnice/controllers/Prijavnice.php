<!-- Prijavnice - controller - 20.03.2017. -->
<?php defined("BASEPATH") OR exit("No direct script access allowed");
class Prijavnice extends CI_Controller {
  function __construct() {
    parent::__construct();
    $this->load->model("Prijavnice_model");
    if(true==1){
      is_login();
  ...
  public function index() {
  		if(CheckPermission("prijavnice", "all_read,own_read")){
			$data["view_data"]=$this->Prijavnice_model->get_data();
  ...
  public function add_edit() {
		...
		if($this->input->post('id')) {
			...
			$this->Prijavnice_model->updateRow('prijavnice', 'prijavnice_id', $this->input->post('id'), $data);
      ...
      if ($afftectedRows > 0 ) {
        ...
        redirect('prijavnice');
			...
			$this->Prijavnice_model->insertRow('prijavnice', $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0 ) {
  ...
  public function get_modal() {
    if($this->input->post('id')){
      $data['data']=$this->Prijavnice_model->Get_data_id($this->input->post('id'));
  ...
  public function get_modal2() {
    if($this->input->post('id')){
      $data['data']=$this->Prijavnice_model->Get_data_id2($this->input->post('id'));
      $data['datas']=$this->Prijavnice_model->Get_data_id3($this->input->post('id'));
  ...
  public function print_record() {
		if($this->input->post('id')){
      $data['data1']=$this->Prijavnice_model->Get_data_id2($this->input->post('id'));
      $data['data2']=$this->Prijavnice_model->Get_data_id3($this->input->post('id'));
  ...
  public function report() {
    $data['data']=$this->Prijavnice_model->Get_data();
  ...
  public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Prijavnice_model->delete_data($value);
		}
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0 ) {
      $this->session->set_flashdata('message', 'Zapis je izbrisan!');
      redirect(base_url().'prijavnice', 'refresh');
    } else {
  ...
  public function delete_data($id) {
		$this->Prijavnice_model->delete_data($id);
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0 ) {
      $this->session->set_flashdata('message', 'Zapis je izbrisan!');
      redirect('prijavnice');
  ...
  public function ajx_data(){
    $primaryKey='prijavnice_id';
    $table='prijavnice';
      $columns = array(
        array('db' => '`prijavnice`.`prijavnice_id`', 'dt' => 0, 'field' => 'prijavnice_id'),
        array('db' => '`prijavnice`.`rb_prijave`', 'dt' => 1, 'field' => 'rb_prijave'),
        array('db' => '`evidencija_korisnika`.`prezime_i_ime`', 'dt' => 2, 'field' => 'prezime_i_ime'),
        array('db' => '`prijavnice`.`prijavnice_id`', 'dt' => 3, 'field' => 'prijavnice_id' ));
        $joinQuery="FROM prijavnice LEFT JOIN `` AS `0` ON (`0`.`_id` = `prijavnice`.`prezime_i_ime`)";
    ...
    public function getFilterdata(){
      ...
      if($this->input->post('dateRange')) {
        ...
        $where="DATE_FORMAT(`prijavnice`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND DATE_FORMAT(`prijavnice`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
      }
      $data["view_data"]=$this->Prijavnice_model->get_data($where);
