<!-- Opći inventar - controller - 15.03.2017. -->
<?php defined("BASEPATH") OR exit("No direct script access allowed");
class Opci_inventar extends CI_Controller {
  function __construct() {
    parent::__construct();
    $this->load->model("Opci_inventar_model");
    if(true==1){
      is_login();
  ...
  public function index() {
    if(CheckPermission("opci_inventar", "all_read,own_read")){
      $data["view_data"]= $this->Opci_inventar_model->get_data();
  ...
  public function add_edit() {
    $data = $this->input->post();
        ...
        if(is_array($dvalue)) {
			...
			$this->Opci_inventar_model->updateRow('opci_inventar', 'opci_inventar_id', $this->input->post('id'), $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0 ) {
        $this->session->set_flashdata('message', 'Zapis je ažuriran!');
        redirect('opci_inventar');
      } else {
        ...
        redirect('opci_inventar');
			...
			$this->Opci_inventar_model->insertRow('opci_inventar', $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0 ) {
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']= $this->Opci_inventar_model->Get_data_id($this->input->post('id'));
  ...
  public function report() {
    $data['data']=$this->Opci_inventar_model->Get_data();
  ...
  public function delete($ids) {
    ...
    foreach ($idsArr as $key => $value) {
      $this->Opci_inventar_model->delete_data($value);
    }
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0 ) {
  ...
  public function delete_data($id) {
    $this->Opci_inventar_model->delete_data($id);
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0 ) {
      $this->session->set_flashdata('message', 'Zapis je izbrisan!');
      redirect('opci_inventar');
  ...
  public function ajx_data(){
    $primaryKey='opci_inventar_id';
    $table='opci_inventar';
    $columns=array(
      array('db' => 'opci_inventar_id', 'dt' => 0),
      array('db' => 'signatura', 'dt' => 1),
      array('db' => 'naziv', 'dt' => 2),
      array('db' => 'vremenski_raspon', 'dt' => 3),
      array('db' => 'opci_inventar_id', 'dt' => 4)
    );
		$joinQuery='';
    ...
    public function getFilterdata(){
		...
		if($this->input->post('dateRange')) {
			...
			$where=" DATE_FORMAT(`opci_inventar`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND DATE_FORMAT(`opci_inventar`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
		}
    $data["view_data"]=$this->Opci_inventar_model->get_data($where);
