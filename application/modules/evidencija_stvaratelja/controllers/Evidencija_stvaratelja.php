<!-- Evidencija stvaratelja - controller - 17.03.2017. -->
<?php defined("BASEPATH") OR exit("No direct script access allowed");
class Evidencija_stvaratelja extends CI_Controller {
  function __construct() {
    parent::__construct();
    $this->load->model("Evidencija_stvaratelja_model");
    if(true==1){
      is_login();
  ...
  public function index() {
    if(CheckPermission("evidencija_stvaratelja", "all_read,own_read")){
			$data["view_data"]= $this->Evidencija_stvaratelja_model->get_data();
  ...
  public function add_edit() {
    $data=$this->input->post();
		...
		if($this->input->post('id')) {
			...
			$this->Evidencija_stvaratelja_model->updateRow('evidencija_stvaratelja', 'evidencija_stvaratelja_id', $this->input->post('id'), $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows>0) {
        $this->session->set_flashdata('message', 'Zapis je ažuriran!');
        redirect('evidencija_stvaratelja');
      } else {
        ...
        redirect('evidencija_stvaratelja');
			...
			$this->Evidencija_stvaratelja_model->insertRow('evidencija_stvaratelja', $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0) {
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']=$this->Evidencija_stvaratelja_model->Get_data_id($this->input->post('id'));
  ...
  public function get_modal2() {
    if($this->input->post('id')){
      $data['data']=$this->Evidencija_stvaratelja_model->Get_data_id2($this->input->post('id'));
  ...
  public function report() {
    $data['data']=$this->Evidencija_stvaratelja_model->Get_data();
  ...
  public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Evidencija_stvaratelja_model->delete_data($value);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0) {
        $this->session->set_flashdata('message', 'Zapis je izbrisan!');
        redirect(base_url().'evidencija_stvaratelja', 'refresh');
      } else {
  ...
  public function delete_data($id) {
		$this->Evidencija_stvaratelja_model->delete_data($id);
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0) {
      $this->session->set_flashdata('message', 'Zapis je izbrisan!');
      redirect('evidencija_stvaratelja');
  	...
  	public function ajx_data(){
		$primaryKey='evidencija_stvaratelja_id';
		$table='evidencija_stvaratelja';
		$columns = array(
      array('db' => '`evidencija_stvaratelja`.`evidencija_stvaratelja_id`', 'dt' => 0, 'field' => 'evidencija_stvaratelja_id'),
      array('db' => '`evidencija_stvaratelja`.`rb_upisa`', 'dt' => 1, 'field' => 'rb_upisa'),
      array('db' => '`evidencija_stvaratelja`.`datum_unosa`', 'dt' => 2, 'field' => 'datum_unosa'),
      array('db' => '`evidencija_stvaratelja`.`maticni_broj`', 'dt' => 3, 'field' => 'maticni_broj'),
      array('db' => '`evidencija_stvaratelja`.`naziv`', 'dt' => 4, 'field' => 'naziv'),
      array('db' => '`evidencija_stvaratelja`.`kategorija`', 'dt' => 5, 'field' => 'kategorija'),
      array('db' => '`evidencija_stvaratelja`.`evidencija_stvaratelja_id`', 'dt' => 6, 'field' => 'evidencija_stvaratelja_id'));
      $joinQuery="FROM evidencija_stvaratelja";
  	...
  	public function getFilterdata(){
		...
		if($this->input->post('dateRange')) {
			...
			$where="DATE_FORMAT(`evidencija_stvaratelja`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND DATE_FORMAT(`evidencija_stvaratelja`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
		}
		$data["view_data"]=$this->Evidencija_stvaratelja_model->get_data($where);
