<!-- Knjiga snimljenoga gradiva - controller - 16.03.2017. -->
<?php defined("BASEPATH") OR exit("No direct script access allowed");
class Knjiga_snimljenoga_gradiva extends CI_Controller {
  function __construct() {
    parent::__construct();
    $this->load->model("Knjiga_snimljenoga_gradiva_model");
		if(true==1){
			is_login();
  ...
  public function index() {
    if(CheckPermission("knjiga_snimljenoga_gradiva", "all_read,own_read")){
      $data["view_data"]= $this->Knjiga_snimljenoga_gradiva_model->get_data();
  ...
  public function add_edit() {
    $data = $this->input->post();
		...
		if($this->input->post('id')) {
			...
			$this->Knjiga_snimljenoga_gradiva_model->updateRow('knjiga_snimljenoga_gradiva', 'knjiga_snimljenoga_gradiva_id', $this->input->post('id'), $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows>0) {
        $this->session->set_flashdata('message', 'Zapis je ažuriran!');
        redirect('knjiga_snimljenoga_gradiva');
      } else {
        ...
        redirect('knjiga_snimljenoga_gradiva');
			...
			$this->Knjiga_snimljenoga_gradiva_model->insertRow('knjiga_snimljenoga_gradiva', $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0) {
  ...
  public function get_modal() {
    if($this->input->post('id')){
			$data['data']=$this->Knjiga_snimljenoga_gradiva_model->Get_data_id($this->input->post('id'));
  ...
  public function report() {
    $data['data']=$this->Knjiga_snimljenoga_gradiva_model->Get_data();
  ...
  public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Knjiga_snimljenoga_gradiva_model->delete_data($value);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0) {
        $this->session->set_flashdata('message', 'Zapis je izbrisan!');
        redirect(base_url().'knjiga_snimljenoga_gradiva', 'refresh');
      } else {
  ...
  public function delete_data($id) {
		$this->Knjiga_snimljenoga_gradiva_model->delete_data($id);
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0) {
      $this->session->set_flashdata('message', 'Zapis je izbrisan!');
      redirect('knjiga_snimljenoga_gradiva');
  ...
  public function ajx_data(){
		$primaryKey='knjiga_snimljenoga_gradiva_id';
		$table='knjiga_snimljenoga_gradiva';
		$columns=array(
      array('db' => '`knjiga_snimljenoga_gradiva`.`knjiga_snimljenoga_gradiva_id`', 'dt' => 0),
      array('db' => '`knjiga_snimljenoga_gradiva`.`rb_upisa`', 'dt' => 1),
      array('db' => '`knjiga_snimljenoga_gradiva`.`signatura`', 'dt' => 2),
      array('db' => '`knjiga_snimljenoga_gradiva`.`naziv_izvornika`', 'dt' => 3),
      array('db' => '`knjiga_snimljenoga_gradiva`.`knjiga_snimljenoga_gradiva_id`', 'dt' => 4)
    );
		$joinQuery='';
      ...
      if($this->input->post('dateRange')) {
        ...
        $where="DATE_FORMAT(`knjiga_snimljenoga_gradiva`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND DATE_FORMAT(`knjiga_snimljenoga_gradiva`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
      }
      $data["view_data"]=$this->Knjiga_snimljenoga_gradiva_model->get_data($where);
