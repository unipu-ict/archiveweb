<!-- Akvizicije - controller - 13.03.2017. -->
<?php defined("BASEPATH") OR exit("No direct script access allowed");
class Akvizicije extends CI_Controller {
  function __construct() {
    parent::__construct();
    $this->load->model("Akvizicije_model");
    if(true==1){
      is_login();
  ...
  public function index(){
    if(CheckPermission("akvizicije", "all_read,own_read")){
      $data["view_data"]=$this->Akvizicije_model->get_data();
  ...
  if($this->input->post('id')) {
    ...
    $this->Akvizicije_model->updateRow('akvizicije', 'akvizicije_id', $this->input->post('id'), $data);
    ...
    unset($data['submit']);
      ...
      $this->Akvizicije_model->insertRow('akvizicije', $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0 ) {
  ...
  public function get_modal() {
    if($this->input->post('id')){
      $data['data']=$this->Akvizicije_model->Get_data_id($this->input->post('id'));
  ...
  public function get_modal2() {
		if($this->input->post('id')){
      $data['data']=$this->Akvizicije_model->Get_data_id2($this->input->post('id'));
  ...
  public function report() {
    $data['data']=$this->Akvizicije_model->Get_data();
  ...
  public function delete($ids) {
    ...
    foreach ($idsArr as $key => $value) {
      $this->Akvizicije_model->delete_data($value);
    }
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0 ) {
      $this->session->set_flashdata('message', 'Zapis je izbrisan!');
      redirect(base_url().'akvizicije', 'refresh');
    } else {
  ...
  public function delete_data($id) {
		$this->Akvizicije_model->delete_data($id);
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0 ) {
      $this->session->set_flashdata('message', 'Zapis je izbrisan!');
      redirect('akvizicije');
    ...
    public function ajx_data(){
      $primaryKey='akvizicije_id';
      $table='akvizicije';
      $columns=array(
        array('db' => '`akvizicije`.`akvizicije_id`', 'dt' => 0, 'field' => 'akvizicije_id'),
        array('db' => '`akvizicije`.`rb_u_godini`', 'dt' => 1, 'field' => 'rb_u_godini'),
        array('db' => '`akvizicije`.`datum_preuzimanja`', 'dt' => 2, 'field' => 'datum_preuzimanja'),
        ...
        array('db' => '`akvizicije`.`akvizicije_id`', 'dt' => 5, 'field' => 'akvizicije_id'));
        $joinQuery="FROM akvizicije LEFT JOIN `` AS `0` ON (`0`.`_id` = `akvizicije`.`predavatelj`)
        LEFT JOIN `` AS `1` ON (`1`.`_id` = `akvizicije`.`stvaratelj`)";
        ...
        if(CheckPermission($table, "all_read")){}
          ...
          public function getFilterdata(){
            ...
            if($this->input->post('dateRange')) {
              ...
              $where="DATE_FORMAT(`akvizicije`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND DATE_FORMAT(`akvizicije`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
            }
            $data["view_data"]=$this->Akvizicije_model->get_data($where);
