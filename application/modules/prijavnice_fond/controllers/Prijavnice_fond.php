<!-- Prijavnice_fond - controller - 29.03.2017. -->
<?php defined("BASEPATH") OR exit("No direct script access allowed");
class Prijavnice_fond extends CI_Controller {
  function __construct() {
    parent::__construct();
    $this->load->model("Prijavnice_fond_model");
    if(true==1){
      is_login();
  ...
  public function index() {
  		if(CheckPermission("prijavnice_fond", "all_read,own_read")){
			$data["view_data"]=$this->Prijavnice_fond_model->get_data();
  ...
  public function add_edit() {
		...
		if($this->input->post('id')) {
			...
			$this->Prijavnice_fond_model->updateRow('prijavnice_fond', 'prijavnice_fond_id', $this->input->post('id'), $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0 ) {
        $this->session->set_flashdata('message', 'Zapis je ažuriran!');
        redirect('prijavnice_fond');
      } else {
        ...
        redirect('prijavnice_fond');
			...
			$this->Prijavnice_fond_model->insertRow('prijavnice_fond', $data);
      $afftectedRows=$this->db->affected_rows();
      if ($afftectedRows > 0 ) {
  ...
  public function get_modal() {
    if($this->input->post('id')){
      $data['data']=$this->Prijavnice_fond_model->Get_data_id($this->input->post('id'));
  ...
  public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Prijavnice_fond_model->delete_data($value);
		}
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0 ) {
      $this->session->set_flashdata('message', 'Zapis je izbrisan!');
      redirect(base_url().'prijavnice_fond', 'refresh');
    } else {
  ...
  public function delete_data($id) {
		$this->Prijavnice_fond_model->delete_data($id);
    $afftectedRows=$this->db->affected_rows();
    if ($afftectedRows > 0 ) {
      $this->session->set_flashdata('message', 'Zapis je izbrisan!');
      redirect('prijavnice_fond');
  ...
  public function ajx_data(){
    $primaryKey='prijavnice_fond_id';
    $table='prijavnice_fond';
      $columns = array(
        array('db' => '`prijavnice_fond`.`prijavnice_fond_id`', 'dt' => 0, 'field' => 'prijavnice_fond_id'),
        array('db' => '`prijavnice_fond`.`signatura`', 'dt' => 1, 'field' => 'signatura'),
        array('db' => '`prijavnice_fond`.`id_prijave`', 'dt' => 2, 'field' => 'id_prijave'),
        array('db' => '`prijavnice_fond`.`prijavnice_fond_id`', 'dt' => 3, 'field' => 'prijavnice_fond_id' ));
        $joinQuery="FROM prijavnice_fond";
    ...
    public function getFilterdata(){
      ...
      if($this->input->post('dateRange')) {
        ...
        $where="DATE_FORMAT(`prijavnice_fond`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND DATE_FORMAT(`prijavnice_fond`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
      }
      $data["view_data"]=$this->Prijavnice_fond_model->get_data($where);
